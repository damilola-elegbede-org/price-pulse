...
- **launchd cron** (`launchd/bareclaude.price-pulse.poll.plist`): daily pipeline run at 07:00 MT via `dist/pipeline.js`. Uses `/opt/homebrew/bin/node` (ARM Homebrew path). `DB_PATH` points to `finn/.state/price-pulse.db` (outside the working tree). `TELEGRAM_SEND_SCRIPT` path is validated against the BareClaude root at startup.
